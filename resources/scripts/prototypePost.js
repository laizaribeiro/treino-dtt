...
//        var y = JSON.stringify(axEventObject);
        ...
        if ( (controle != 1) && (localStorage.codigo != "") && (localStorage.nome != "") && ($jobj('u8_input').val() != "") && (localStorage.terapeuta != "") ){
         
      
          $.ajax({
              type: 'POST',
              async: false,
              url: 'http://www.caed-lab.com/laiza/brasil/treino-dtt/api_dados.php',
              data: {
                'codigo': localStorage.codigo, // código do aluno
                'nome': localStorage.nome, //  nome do aluno
                 'n_sessao': $jobj('u8_input').val(), // numero da sessao
                'terapeuta': localStorage.terapeuta
              },
              success: function(){
                //alert('Funcionou');
              },
         ...
         //treino-dtt
         //armazena o nome e o instante no qual o botão foi clicado.
        var instanteAtual = Date();

          $.ajax({
            type: 'POST',
            async: true,
            url: 'http://www.caed-lab.com/laiza/brasil/treino-dtt/api_clique.php',
            data: {
              'codigoAluno': localStorage.codigo, // código do aluno
              'nomeVariavel': axObjLabel, // nome do botão clicado
              'instante': instanteAtual // instante no qual o botão foi clicado
        ...
        query.each(function(dObj, elementId) {
            ...
            if(($ax.public.fn.IsTextArea(dObj.type) || $ax.public.fn.IsTextBox(dObj.type)) && hasPlaceholder) {
                ...
                inputJobj.bind('focus', function () {
				  ...
				  //treino-dtt
				  localStorage.setItem('codigo', $jobj('u10_input').val());
                  localStorage.setItem('nome', $jobj('u6_input').val());
                  localStorage.setItem('terapeuta', $jobj('u7_input').val());
                  localStorage.setItem('n_sessao', $jobj('u8_input').val());
    ...
    var _processTempInit = function() {
        ...
        $axure.messageCenter.addMessageListener(function(message, data) {
            if(message == 'setGlobalVar') {
                ...
                //treino-dtt
                //armazena o codigo do aluno, nome da variável e o valor de cada uma
               if (data.globalVarValue != 0){
                  $.ajax({
                    type: 'POST',
                    async: true,
                    url: 'http://www.caed-lab.com/laiza/brasil/treino-dtt/api_variavel.php',
                    data: {
                      'codigo': localStorage.codigo, // código do aluno
                      'nome_variavel': data.globalVarName, // nome da variável
                      'valor': data.globalVarValue // valor da variável
                    },
                    success: function(){
                      //alert('Funcionou');
                    },
